...
---
= Parámetros de entrada para Upgrade Health Checker
:hardbreaks:
:allow-uri-read: 
:icons: font
:linkattrs: 
...
El parámetro  `--dashboard` regenera el panel de control a partir de ejecuciones existentes en  `<runs>/dashboard/index.html` sin contactar con un clúster. Lo usas para actualizar la vista de la flota fuera de una ejecución normal. El panel de control ofrece una visión a grandes rasgos del estado de salud de todos los clústeres evaluados recientemente, lo que te permite supervisar y comparar fácilmente la preparación para la actualización de varios clústeres en un solo lugar. Puedes abrir el panel de control en un navegador web para ver de un vistazo el estado de salud, los posibles bloqueadores y los riesgos de cada clúster.

* Argumento de CLI: `--dashboard`
